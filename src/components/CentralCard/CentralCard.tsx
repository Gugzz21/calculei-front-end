...
function CentralCard({ form, juros, today, loading, erro, isFormValid, onFormChange, onJurosChange, onCalcular, onLimpar }: CentralCardProps) {
  ...
  return (
    <div className="flex flex-col mx-auto bg-slate-50 rounded-lg pb-6 w-[95%] md:w-full max-w-[1200px] h-auto md:ml-95 p-4 md:p-6 gap-5 shadow-sm border border-slate-400">
      <div className="text-gray-600 font-bold text-2xl md:text-3xl mb-4 md:mb-6"><h1 className="underline">Atualização Monetária</h1>
      </div>

      {/* Linha 1: Tipo de Cálculo + Índice de Correção */}
      <div className="flex flex-col md:flex-row gap-6 w-full">
        <div className="flex-1 w-full">
          <TipoCalculo value={form.tipoCalculo} onChange={(v) => onFormChange('tipoCalculo', v)} />
        </div>
        <div className="flex-1 w-full">
          <IndiceCorrecao value={form.indiceCorrecao} onChange={(v) => onFormChange('indiceCorrecao', v)} />
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start md:items-end w-full">
        <div className="w-full md:w-auto">
          <InputValor value={form.valor} onChange={(v) => onFormChange('valor', v)} />
        </div>
        ...
        <div className="w-full md:w-auto"><Descricao value={form.descricao} onChange={(v) => onFormChange('descricao', v)} /></div>
      </div>

      <div className="text-[12px] mt-2 md:mt-4 text-gray-400">Todos os valores estão em Reais (R$) A presente calculadora virtual não substitui o cálculo realizado e homologado judicialmente.</div>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6 justify-end w-full">
        <Calcular onClick={onCalcular} loading={loading} disabled={!isFormValid} />
